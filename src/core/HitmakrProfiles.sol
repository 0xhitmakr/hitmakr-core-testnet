 ...
 * @notice This contract manages Hitmakr user profiles, represented as non-transferable (SoulBound) ERC721 tokens.
 * Each profile is linked to a unique username and contains on-chain metadata rendered as an SVG image.
 * The contract prioritizes gas efficiency and security.
 */
contract HitmakrProfiles is ERC721 {
    ...
    /*
     * Constants defining username length restrictions.
     */
    uint256 private immutable MINIMUM_LENGTH = 3;
    uint256 private immutable MAXIMUM_LENGTH = 30;

    /*
     * Counter for the total number of profiles created.
     */
    uint256 private _profileCount;
    
    /*
     * Mapping: Hash of username to the owner's address.  Using a hash saves gas compared to storing the full string.
     * Mapping: User address to a boolean indicating whether they have a profile.
     * Mapping: User address to their username string.  This is redundant but improves gas efficiency for lookups.
     */
    mapping(bytes32 => address) private _profileByNameHash;
    mapping(address => bool) private _hasProfile;
    mapping(address => string) private _nameByAddress;

    /*
     * Event emitted when a new profile is created. Indexed parameters allow for efficient filtering.
     */
    event ProfileCreated(address indexed user, string name, uint256 indexed tokenId);
    
    /*
     * SVG components for the dynamic profile image. Stored as constant byte strings to reduce gas costs.
     * These are concatenated in `_createSVG` to form the complete image.
     */
    ...
    bytes private constant SVG_USERNAME_SUFFIX = bytes('</text> <text x="415" y="230px" font-size="14" fill="#fff" text-anchor="end" style="font-family: \'Quicksand\', sans-serif;font-weight:700;">');
    bytes private constant SVG_SUFFIX = bytes('</text> <defs> <linearGradient id="paint0_linear_170_24" x1="0" y1="0" x2="220" y2="147.5" gradientUnits="userSpaceOnUse"> <stop stop-color="#302071"/> <stop offset="1" stop-color="#191541"/> </linearGradient> <linearGradient id="paint1_linear_170_24" x1="370.232" y1="50.4785" x2="403.702" y2="50.4785" gradientUnits="userSpaceOnUse"> <stop stop-color="#7AC68F"/> <stop offset="0.5147" stop-color="#2CBCEE"/> <stop offset="1" stop-color="#6E5DA8"/> </linearGradient> </defs> </svg>');

    /**
     * @notice Constructor initializes the ERC721 contract with the token name and symbol.
     */
    constructor() ERC721("Hitmakr Profiles", "HMP") {}

    /**
     * @dev Internal helper function to check if a character is a lowercase alphanumeric character.
     * Reduces gas costs by avoiding string comparisons.
     * @param char The character to validate.
     * @return True if the character is valid, false otherwise.
     */
    function _isValidChar(bytes1 char) private pure returns (bool) {
        return (char >= 0x30 && char <= 0x39) || (char >= 0x61 && char <= 0x7A);
    }

    /**
     * @dev Internal helper function to validate the username format. Ensures it contains only
    ...
    function _validateUsername(string calldata name) private pure returns (bool) {
        ...
        for(uint256 i; i < len;) {
            ...
            unchecked { ++i; } // Safe unchecked increment within the loop.
        }
        return true;
    }
    ...
    /**
     * @dev Internal helper function to create a hash of the username.  Hashing is used for efficient storage
     * and comparison of usernames.
     ...
     * Reverts if the username is invalid, already taken, or the user already has a profile.
     * @param name The desired username for the profile. Must be between `MINIMUM_LENGTH` and `MAXIMUM_LENGTH`
    ...
    function register(string calldata name) external {
        ...
        unchecked {
            _profileCount++;  // Safe unchecked increment since overflow is unlikely
        }
        ...
        _safeMint(msg.sender, tokenId); // Mint the SoulBound NFT

        emit ProfileCreated(msg.sender, name, tokenId);
    }

    /**
     * @dev Internal helper function to construct the SVG image for the token URI.  Dynamically generates
     * the SVG based on the username and user address.
     * @param username The username to display in the SVG.
     * @param userAddress The user's address to display in the SVG.
    ...
    /**
     * @notice Returns the token URI for a given token ID.  The URI contains the profile metadata, including
     * the dynamically generated SVG image.
     * @param tokenId The ID of the token.
     * @return The token URI as a string.
     */
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        address userAddress = ownerOf(tokenId);
        string memory username = _nameByAddress[userAddress]; // Redundant storage avoids an extra SLOAD
    ...
    /*
     * Overrides of ERC721 transfer functions to prevent token transfers, enforcing the SoulBound nature
     * of the profile NFTs.
     */
    ...
    /*
     * Overrides of ERC721 approval functions to prevent approvals, since transfers are not allowed.
     */
     ...
     * @return The address associated with the username, or address(0) if no profile exists for that name.
     */
    function profileAddressByName(string calldata name) external view returns (address) {
        return _profileByNameHash[_createNameHash(name)];
    }

    /**
     * @notice  Checks if a given address has already registered a profile.
     ...
     * @return The username associated with the address, or an empty string if no profile exists for that address.
     */
    function nameByAddress(address user) external view returns (string memory) {
        return _nameByAddress[user];
    }
